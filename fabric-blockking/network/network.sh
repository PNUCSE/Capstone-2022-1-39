...
function deployCC() {
  scripts/deployCC1.sh trade asset ../chaincode/asset/chaincode-go/ go 1.0 1 $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
  scripts/deployCC1.sh trade wallet ../chaincode/token/chaincode-go/ go 1.0 1 $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
  scripts/deployCC1.sh trade receipt ../chaincode/receipt/chaincode-go/ go 1.0 1 $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
  scripts/deployCC2.sh validation validation ../chaincode/validation/chaincode-go/ go 1.0 1 $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.14.49.119.248 docker_peer0.org1.14.49.119.248 docker_peer0.org2.14.49.119.248 docker_peer0.org3.14.49.119.248 docker_peer0.org4.14.49.119.248
...
if [ "$MODE" == "up" ]; then
  ...
  infoln "Creating channel 'validation and trade'."
